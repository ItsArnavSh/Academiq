...
import { doc, collection, setDoc, getDoc, updateDoc, getDocs } from "firebase/firestore"
...
async function addStudentToContestSubcollection(contestid: string, studentEmail: string, questionCount: number) {
  try {
    // Reference the subcollection "students" within the specific contest document
    const studentRef = doc(collection(db, `Contests/${contestid}/students`), studentEmail)
...
export default function ContestDetails() {
  ...
  const handleButtonClick = () => {
    ...
    if (contest) {
      ...
      if (studentEmail) {
        addStudentToContestSubcollection(contest.id, studentEmail, contest.questions.length)
          .then(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-50 to-indigo-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="absolute top-4 right-4">
        <Link
          href="/Profile"
          className="bg-white text-blue-600 font-semibold py-2 px-4 rounded-full shadow-md hover:bg-blue-50 transition duration-300"
        >
          Dashboard
        </Link>
      </div>
